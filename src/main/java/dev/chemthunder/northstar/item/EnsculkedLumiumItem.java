...
public class EnsculkedLumiumItem extends Item {
    ...
    public ActionResult useOnBlock(ItemUsageContext context) {
        ...
        if (user != null && user.isSneaking() && state.isOf(Blocks.LAVA_CAULDRON)) {
            ItemStack stack = user.getMainHandStack();
            ItemStack offStack = user.getOffHandStack();

            if (stack.isOf(Items.NETHERITE_SWORD) && offStack.isOf(NorthItems.ENSCULKED_LUMIUM)) {
                offStack.decrement(1);
                stack.decrement(1);
                user.giveItemStack(NorthItems.GRACE.getDefaultStack());
                user.addStatusEffect(new StatusEffectInstance(NorthEffects.RALLY, 120));
                user.playSound(SoundEvents.ITEM_TRIDENT_RETURN, 0.8F, 1.0F);
            }
            return ActionResult.SUCCESS;
        }
